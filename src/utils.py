...
def padding_captions(
    ...
    if discrete_tokens is None:
        ...
        padding_masks = None
        hard_prompts_length = []
        max_length = 2 * args.max_num_of_entities - 1 + args.prompt_template_length + captions_tokens.shape[-1]
        for i in range(len(discrete_tokens)):
            tokens = torch.cat((discrete_tokens[i], captions_tokens[i])) # hard prompt + GT caption
            ...
            tokens = tokens[:max_length].unsqueeze(dim = 0)
            ...
            loss_tokens = torch.cat((loss_tokens[1:], torch.zeros((1), dtype = torch.int64))).unsqueeze(dim = 0)
